...
public class EmprestimoDAO {
        ...
        try (Connection conn = ConnectionFactory.getConnection();
            ...
            if (rs.next()) {
            	emp = new Emprestimo(
                    rs.getInt("id"),
                    rs.getInt("conta_id"),
                    rs.getBigDecimal("valor_emprestimo"),
                    rs.getBigDecimal("juros"),
                    rs.getInt("parcelas"),
                    Emprestimo.tipoStatus.valueOf(rs.getString("status")),
                    rs.getTimestamp("data_solicitacao"),
                    rs.getTimestamp("data_aprovacao"),
                    rs.getTimestamp("data_ultimo_pagamento")
                );
            }
    ...
    public void atualizarStatus(int id, Emprestimo.tipoStatus novoStatus) throws DataAccessException {
